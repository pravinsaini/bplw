...
function Validate(){
	...
	if(publisher.value.trim() == ''){
		valid = false;
		message = message + '*Publisher Number is required' + '\n';
	}
	if(edition.value.trim() == ''){
		valid = false;
		message = message + '*Edition is required' + '\n';
	}
	if(year.value.trim() == ''){
		valid = false;
		message = message + '*Year is required' + '\n';
	}
	if(isbn.value.trim() == ''){
		valid = false;
		message = message + '*ISBN Number is required' + '\n';
	}
	if (valid == false){
		alert(message);
		return false;
...
?>
<li>Inter Library Loan - Book Request</li>
</ul>
</div>
</div>
...
<br/>
<div align="center" style="color: #211D70; font-size:18px; font-weight:bold; text-decoration:underline;">Book Request Form</div>
<form action="bookrequest.php" method="post" enctype="multipart/form-data" onSubmit="return Validate();">
						<?php				
$message="";
...
$bitsid=$info['bitsid'];
		$name=$info['name'];
		$email=$info['email'];
				$query = 'insert into ill_book_request(author, title, publisher, edition, year, isbn, date, bitsid, name, email) 
					values("'.$author.'", "'.$title.'", "'.$publisher.'", "'.$edition.'", "'.$year.'", "'.$isbn.'", "'.$date.'", "'.$bitsid.'", "'.$name.'", "'.$email.'")'; 
...
<tr>
								...
								<td><input type="text" name="publisher"  id="publisher" placeholder="&nbsp;" class="text_box"/></td>								 
								</tr>
								<tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>								
					            <tr>	
								<td align="right" style="color: #211D70;">Edition:&nbsp;</td>
								<td><input type="text" name="edition"  id="edition" placeholder="&nbsp;" class="text_box"/></td>					
								</tr>
								<tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
								<tr>	
								<td align="right" style="color: #211D70;">Year of Publication:&nbsp;</td>
								<td><input type="text" name="year"  id="year" placeholder="&nbsp;" class="text_box"/></td>					
								</tr>
								<tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
								<tr>	
								<td align="right" style="color: #211D70;">ISBN No.:&nbsp;</td>
								<td><input type="text" name="isbn"  id="isbn" placeholder="&nbsp;" class="text_box"/></td>					
								</tr>
								<tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;<a href="bookrequest.php" style="padding: 4px 15px 4px 15px; width:80px; background-color :#06588f; color: #FFFFFF; font-weight: bold; border-radius: 5px; font-size:14px;">Reset</a>
</td>
</tr>
</table><br><br><br>
</form>
