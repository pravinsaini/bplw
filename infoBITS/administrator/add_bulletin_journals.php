...
function Validate(){
	...
	var journal_url = document.getElementById("journal_url");
	var dep_name = document.getElementById("dep_name");
	var month = document.getElementById("month");
	var journal_cover = document.getElementById("journal_cover");
	...
	if(journal_title.value.trim() == ''){
		valid = false;
		message = message + '*Journal Title is required' + '\n';
	}
	if(journal_url.value.trim() == ''){
		valid = false;
		message = message + '*Journal URL is required' + '\n';
	}
	if(journal_cover.value.trim() == ''){
		valid = false;
		message = message + '*Journal Cover is required' + '\n';
	}
	if (valid == false){
		alert(message);
		return false;
...
?>
<li>infoBITS Bulletin - Add Journals</li>
</ul>
</div>
</div>
...
<tr style="line-height: 27px">
			...
			<tr><td align="left" style="padding:5px; border-bottom:1px solid #4a9ace;"><img src="../images/list_style_type_icon.png" style="width:12px; height:12px; vertical-align:middle;"/>&nbsp;<a href="add_bulletin_books.php" style="background-color: #FFFFFF; color: #06588f; font-weight: bold; font-size:12px;">Add Books</a></td></tr>
			<tr><td align="left" style="padding:5px; border-bottom:1px solid #4a9ace;"><img src="../images/list_style_type_icon.png" style="width:12px; height:12px; vertical-align:middle;"/>&nbsp;<a href="" style="background-color: #FFFFFF; color: #06588f; font-weight: bold; font-size:12px;">Add Journals</a></td></tr>
...
<tr style="line-height: 27px">
			<td style="background-color: #06588f; color: #FFFFFF; font-weight: bold; font-size:13px; border-top-left-radius:4px; border-top-right-radius:4px;" align="center">Add Journals</td></tr></table>
			<br/>
<div class="hidden_box_right_panel">
<form action="add_bulletin_journals.php" method="post" enctype="multipart/form-data" onSubmit="return Validate();">
<?php				
$message="";
...
$journal_number = trim($_POST['Journal_num']);
$journal_type = trim($_POST['Journal_type']);
$name=trim($_POST['dep_name']);

if($_FILES['journal_cover']['type'] != 'image/jpeg'
				&&  $_FILES['journal_cover']['type'] != 'image/jpg'
				&&  $_FILES['journal_cover']['type'] != 'image/gif'
				&& $_FILES['journal_cover']['type'] != 'image/png')
			{
				$message ='Please upload only Image file';
			}
		else if($_FILES['journal_cover']['size'] >4194304)
			{
	...
	if($x == 0)
	{
		if($journal_number=="Journal - 1")
		{
				...
				if($z > 0)
				{
						...
						$filetmp = $_FILES["journal_cover"]["tmp_name"];						
						$file_name = $_FILES["journal_cover"]["name"];
						...
						$sql = 'INSERT INTO bulletin(name, code, tc1, j1, jurl1, Month, Year, issue_number, volume_number, Journal_one, journal_type_one)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$volume_number['volume_number'].'", "'.$journal_number.'", "'.$journal_type.'")'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
							}
					$message ='Journal - 1 Details added Successfully!!!';
				}
				else
				{
						$rand_date=date('dmYHis');
						...
						$sql = 'INSERT INTO bulletin(name, code, tc1, j1, jurl1, Month, Year, issue_number, volume_number, journal_one, journal_type_one)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$new_volume_number.'", "'.$journal_number.'", "'.$journal_type.'")';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
		...
		if($journal_number=="Journal - 2")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'INSERT INTO bulletin(name, code, tc2, j2, jurl2, Month, Year, issue_number, volume_number, Journal_two, Journal_type_two)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$volume_number['volume_number'].'", "'.$journal_number.'", "'.$journal_type.'")'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
							}
					$message ='Journal - 2 Details added Successfully!!!';
				}
				else
				{
						$rand_date=date('dmYHis');
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'INSERT INTO bulletin(name, code, tc2, j2, jurl2, Month, Year, issue_number, volume_number, Journal_two, Journal_type_two)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$new_volume_number.'", "'.$journal_number.'", "'.$journal_type.'")';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
		...
		if($journal_number=="Journal - 3")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'INSERT INTO bulletin(name, code, tc3, j3, jurl3, Month, Year, issue_number, volume_number, Journal_three, Journal_type_three)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$volume_number['volume_number'].'", "'.$journal_number.'", "'.$journal_type.'")'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
							}
					$message ='Journal - 3 Details added Successfully!!!';
				}
				else
				{
						$rand_date=date('dmYHis');
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'INSERT INTO bulletin(name, code, tc3, j3, jurl3, Month, Year, issue_number, volume_number, Journal_three, Journal_type_three)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$new_volume_number.'", "'.$journal_number.'", "'.$journal_type.'")';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
		...
		if($journal_number=="Journal - 4")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'INSERT INTO bulletin(name, code, tc4, j4, jurl4, Month, Year, issue_number, volume_number, Journal_four, Journal_type_four)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$volume_number['volume_number'].'", "'.$journal_number.'", "'.$journal_type.'")'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
							}
					$message ='Journal - 4 Details added Successfully!!!';
				}
				else
				{
						$rand_date=date('dmYHis');
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'INSERT INTO bulletin(name, code, tc4, j4, jurl4, Month, Year, issue_number, volume_number, Journal_four, Journal_type_four)VALUES("'.$name.'", "'.$code.'", "'.$filename.'", "'.$journal_title.'", "'.$journal_url.'", "'.$month.'", "'.$year.'", "'.$issue_number['issue_number'].'", "'.$new_volume_number.'", "'.$journal_number.'", "'.$journal_type.'")';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
	...
	else
	{
		if($journal_number=="Journal - 1")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc1="'.$filename.'", j1="'.$journal_title.'", jurl1="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$volume_number['volume_number'].'", Journal_one="'.$journal_number.'", Journal_type_one="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
				...
				else
				{
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc1="'.$filename.'", j1="'.$journal_title.'", jurl1="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$new_volume_number.'", Journal_one="'.$journal_number.'", Journal_type_one="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
		...
		if($journal_number=="Journal - 2")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc2="'.$filename.'", j2="'.$journal_title.'", jurl2="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$volume_number['volume_number'].'", Journal_two="'.$journal_number.'", Journal_type_two="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"';  
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
				...
				else
				{
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc2="'.$filename.'", j2="'.$journal_title.'", jurl2="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$new_volume_number.'", Journal_two="'.$journal_number.'", Journal_type_two="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
							}
					$message ='Journal - 2 Details Updated Successfully!!!';
					}						
		}
		if($journal_number=="Journal - 3")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc3="'.$filename.'", j3="'.$journal_title.'", jurl3="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$volume_number['volume_number'].'", Journal_three="'.$journal_number.'", Journal_type_three="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
				...
				else
				{
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc3="'.$filename.'", j3="'.$journal_title.'", jurl3="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$new_volume_number.'", Journal_three="'.$journal_number.'", Journal_type_three="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
		...
		if($journal_number=="Journal - 4")
		{
				...
				if($z > 0)
				{
						...
						move_uploaded_file($filetmp,$filepath);
						$volume_number=mysql_fetch_array(mysql_query('select volume_number from bulletin where Year="'.$year.'"'));
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc4="'.$filename.'", j4="'.$journal_title.'", jurl4="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$volume_number['volume_number'].'", Journal_four="'.$journal_number.'", Journal_type_four="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"'; 
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
				...
				else
				{
						...
						$volume_number=$v['volume_number'];
						$new_volume_number=$volume_number+1;
						$sql = 'update bulletin set name="'.$name.'", code="'.$code.'", tc4="'.$filename.'", j4="'.$journal_title.'", jurl4="'.$journal_url.'", Month="'.$month.'", Year="'.$year.'", issue_number="'.$issue_number['issue_number'].'", volume_number="'.$new_volume_number.'", Journal_four="'.$journal_number.'", Journal_type_four="'.$journal_type.'" where code="'.$code.'" and Month="'.$month.'" and Year="'.$year.'"';
						$result=mysql_query($sql);
						if ($result==false)
							{
								die(mysql_error());
...
echo '</label></select>';
			...
			<tr>
				 ...
				 <table style="color: #211D70;">
				 <tr>
				<td style="width:180px;">Select Journal:&nbsp;</td>
				<td>
				<?php  
echo '<label><select class="select_small_box" name="Journal_num">';
    echo "<option>" . 'Journal - 1' ."</option>";
	echo "<option>" . 'Journal - 2' ."</option>";
	echo "<option>" . 'Journal - 3' ."</option>";
	echo "<option>" . 'Journal - 4 ' ."</option>";
echo '</label></select>';
				?></td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Journal Type:&nbsp;</td>
				<td>
				<?php  
echo '<label><select class="select_small_box" name="Journal_type">';
    echo "<option>" . 'eJournal' ."</option>";
...
echo '</label></select>';
			...
			<tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
								<tr>
								 <td>Title&nbsp;</td>
								 <td><input type="text" name="journal_title" id="journal_title" placeholder="&nbsp;" class="text_box"/></td>
								 </tr>
								 <tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
								 <tr>
								 <td>URL&nbsp;</td>
								 <td><input type="text" name="journal_url" id="journal_url" placeholder="&nbsp;" class="text_box"/></td>
								 </tr>
								 <tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
								 <tr>
								 <td>Upload Cover Image&nbsp;</td>
								<td><input type="file" name="journal_cover" id="journal_cover" class="file_upload_box"/></td>								 
								 </tr>
	<tr><td colspan="2" align="center" style="width: 100%;">
		<br/>	
				<input type="submit" name="submit" value="Submit" class="Stylish_button" />
				&nbsp;&nbsp;&nbsp;&nbsp;<a href="add_bulletin_journals.php" style="padding: 4px 15px 4px 15px; width:80px; background-color :#06588f; color: #FFFFFF; font-weight: bold; border-radius: 5px; font-size:14px;">Reset</a>
</td></tr>
</table></form><br/>			
<br/>													 
</div></td>
