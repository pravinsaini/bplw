...
function Validate(){
	...
	var notice_cover = document.getElementById("notice_cover");
	...
	if(notice_cover.value.trim() == ''){
		valid = false;
		message = message + '*Notice Cover Image is required' + '\n';
	}
	if(url.value == ''){
		valid = false;
...
<tr style="line-height: 27px">
			<td style="background-color: #06588f; color: #FFFFFF; font-weight: bold; font-size:13px;" align="center">Notices Home</td></tr>
...
<form action="add_notices.php" method="post" enctype="multipart/form-data" onSubmit="return Validate();">
						<?php				
$message="";
...
$url = trim($_POST['url']);	
if($_FILES['notice_cover']['type'] != 'image/jpeg'
				&&  $_FILES['notice_cover']['type'] != 'image/jpg'
				&&  $_FILES['notice_cover']['type'] != 'image/gif'
				&& $_FILES['notice_cover']['type'] != 'image/png')
			{
				$message ='Please upload only Image file';
			}
		else if($_FILES['notice_cover']['size'] >4194304)
			{
			...
			else
			{
						...
						$filetmp = $_FILES["notice_cover"]["tmp_name"];						
						$file_name = $_FILES["notice_cover"]["name"];
						$ext = pathinfo($file_name, PATHINFO_EXTENSION);
						$filename = $rand_date.".".$ext;
						$filepath = "../uploads/notices/images/".$filename;  
						move_uploaded_file($filetmp,$filepath);			
						$query = 'insert into noticeboard(title, image, link) values("'.$title.'", "'.$filename.'", "'.$url.'")'; 
						$result=mysql_query($query);	
						if ($result==false)
						{
							die(mysql_error());
						}
				$message ='Notice has been Uploaded Successfully!!!';
			}
}
?>
<div align="center">&nbsp;<span class="error"><?php echo $message;?></span></div>
						<table align="center" style="width: 80%;" cellpadding="0" cellspacing="0" >
								 ...
								 <tr>
								 <td align="right" style="color: #211D70;">Upload Notice Image:&nbsp;&nbsp;&nbsp;</td>
								 <td><input type="file" name="notice_cover" id="notice_cover" class="notice_upload"/></td> 
								 </tr>
								 <tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
								 <tr>
				...
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="add_notices.php" style="padding: 4px 15px 4px 15px; width:80px; background-color :#06588f; color: #FFFFFF; font-weight: bold; border-radius: 5px; font-size:14px;">Reset</a>
				</td>
			</tr>
			<tr style="line-height: 20px"><td colspan="2">&nbsp;</td></tr>
			</table></form>													 
